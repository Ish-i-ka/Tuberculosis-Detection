...
# Save class indices for later use in evaluate_model
with open(os.path.join(MODELS_DIR, 'class_indices.txt'), 'w') as f:
    f.write(str(train_generator.class_indices))
    
#Model building

def build_model(base_model_trainable=False, learning_rate=LEARNING_RATE_PHASE1):
...
model = tf.keras.models.load_model(checkpoint_path)  # Load the best model from Phase 1
...
model.summary() # Print model summary of fine tuned model
